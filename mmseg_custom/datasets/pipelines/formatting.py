...
class DefaultFormatBundle(object):
    ...
    def __call__(self, results):
        ...
        if 'gt_cls_label' in results:
            results['gt_cls_label'] = DC(to_tensor(results['gt_cls_label']), stack=True, pad_dims=None)
        return results
...
class ToBinaryMask(object):
    ...
    def __call__(self, results):
        assert 'gt_semantic_seg' in results, \
            f'gt_semantic_seg should be in results'
        gt_semantic_seg = results['gt_semantic_seg']
        assert (gt_semantic_seg.ndim == 2 or gt_semantic_seg.ndim == 3), \
            f'ground truth chanel should be 2 or 3'
