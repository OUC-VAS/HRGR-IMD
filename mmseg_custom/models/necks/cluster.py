...
class Cluster(BaseModule):
    def __init__(self, in_channels: List[int], hidden_channels, out_channels: List[int], spix_index: List[int],
                 n_spix: List[int], n_iter, pos_scale, color_scale, fuse_channels: int, freeze_kernel=False, up_index: List[int] = None):
        super().__init__()
        ...
        self.gamma_node = nn.ParameterList([nn.Parameter(torch.tensor([1.]), requires_grad=True) for _ in self.up_index])
