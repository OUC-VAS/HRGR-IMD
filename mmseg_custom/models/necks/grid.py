...
from .core.layer import HGLayer


@NECKS.register_module()
class Grid(BaseModule):
    def __init__(self, in_channels: List[int], out_channels: List[int], spix_index: List[int],
                 n_spix: List[int], fuse_channels: int,
                 fuse_resolution: Tuple[int, int], up_index: List[int] = None):
        super().__init__()
        ...
        self.layers = nn.ModuleList([HGLayer(n_spix[i]) for i in range(len(spix_index))])
