...
from .core.graph import HeterogeneousHANGrapher
from .core.layer import HLayer


@NECKS.register_module()
class HanHSSN(BaseModule):
    """
    ...
    def __init__(self, in_channels: List[int], hidden_channels, out_channels: List[int], spix_index: List[int],
        ...
        self.grapher = HeterogeneousHANGrapher(
            in_channels=[out_channels[spix_index[i]] for i in spix_index],
            hidden_channels=fuse_channels,
            out_channels=[out_channels[spix_index[i]] for i in spix_index],
            n_layer=len(spix_index))
    ...
    def forward(self, x):
        ...
        adj = spixel3d_to_adj_maxtrix(hard_code, total_n_pixel)

        adj_dict = self.grapher.transform_adj(adj, slice_index)
        node_feat_dict = self.grapher.transform_node(node_feat_list)
        node_feat_list = self.grapher(node_feat_dict, adj_dict)

        for i in range(len(self.up_index)):
            x_index = self.up_index[i]
            B, C, H, W = x[x_index].shape
            node_feat = node_feat_list[i]

            # node_feat_slice = self.reduces_node[i](node_feat_slice)

            feat = affine_list[i] @ (node_feat * self.gamma_node[i])
