...
from .core.graph import HeterogeneousGrapher
from .core.graphv2 import HeterogeneousGrapherV2
from .core.graphv3 import HeterogeneousGrapherV3
from .core.layer import HLayer


@NECKS.register_module()
class HeterHSSN(BaseModule):
    """
    ...
    def __init__(self, in_channels: List[int], hidden_channels, out_channels: List[int], spix_index: List[int],
        ...
        self.graph_conv_node = HeterogeneousGrapher(fuse_channels, fuse_channels * 2, fuse_channels, self.n_layers)
    ...
    def forward(self, x):
        ...
        node_feat = self.graph_conv_node(node_feat, adj, slice_index)
...
@NECKS.register_module()
class HeterHSSNV2(HeterHSSN):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        fuse_channels = self.fuse_channels
        self.graph_conv_node = HeterogeneousGrapherV2(fuse_channels, fuse_channels * 2, fuse_channels, self.n_layers)


@NECKS.register_module()
class HeterHSSNV3(HeterHSSN):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        fuse_channels = self.fuse_channels
        self.graph_conv_node = HeterogeneousGrapherV3(fuse_channels, fuse_channels * 2, fuse_channels, self.n_layers)
